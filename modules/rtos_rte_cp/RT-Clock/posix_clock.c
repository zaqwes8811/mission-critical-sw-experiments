...
void print_scheduler(void) {
    const int schedType = sched_getscheduler(getpid());
...
void end_delay_test(void) {
  ...
  printf("MY_CLOCK clock stop seconds = %ld, nanoseconds = %ld\n", 
    ...
    printf("MY_CLOCK clock DT seconds = %ld, msec=%ld, usec=%ld, nsec=%ld, sec=%6.9lf\n", rtclk_dt.tv_sec,
           rtclk_dt.tv_nsec / 1000000, rtclk_dt.tv_nsec / 1000, rtclk_dt.tv_nsec, real_dt);
...
void main_local(void) {
    ...
    if (pthread_attr_destroy(&main_sched_attr) != 0) perror("attr destroy");
#else
    delay_test((void *)0);
#endif
