...
void *Sequencer(void *threadp) {
    ...
    do {
        ...
        delay_time.tv_nsec = std_delay_time.tv_nsec - (scale_dt * (NANOSEC_PER_SEC + DT_SCALING_UNCERTAINTY_NANOSEC)) -
                             CLOCK_BIAS_NANOSEC;
        // syslog(LOG_CRIT, "RTSEQ: scale dt=%lf @ sec=%lf after=%lf with dt=%lf\n", scale_dt, current_time, last_time,
        // delta_t);
#else
        ...
        // scale_dt, current_time_val.tv_sec, current_time_val.tv_nsec, delay_time.tv_sec, delay_time.tv_nsec);
#endif
...
void get_cpu_core_config(void) {
    ...
    if (rc != 0)
        ...
        for (idx = 0; idx < CPU_SETSIZE; idx++)
            if (CPU_ISSET(idx, &cpuset)) printf(" %d", idx);
